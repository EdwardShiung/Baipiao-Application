...
public interface OrganizationRepository extends JpaRepository<Organization, Long> {
    ...
    @Query(value = "SELECT * FROM organizations WHERE email = :email", nativeQuery = true)
    Organization findByEmail(@Param("email") String email);

    // Example for filtering by address
    @Query(value = "SELECT * FROM organizations WHERE address LIKE %:address%", nativeQuery = true)
    List<Organization> findByAddressContaining(@Param("address") String address);

    // Example for filtering by phone number
    @Query(value = "SELECT * FROM organizations WHERE phoneno = :phoneno", nativeQuery = true)
